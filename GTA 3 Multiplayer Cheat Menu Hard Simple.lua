...
menu:addButton("Freeze Player", function()
    -- Get the player ID from the user
    local playerID = input("Enter player ID (0-7): ")
    -- Write 0x01 to the player's freeze address
    writeBytes(0x004ZYXWVU + playerID * 0x100, {0x01})
end)

menu:addButton("Unfreeze Player", function()
    -- Get the player ID from the user
    local playerID = input("Enter player ID (0-7): ")
    -- Write 0x00 to the player's freeze address
    writeBytes(0x004ZYXWVU + playerID * 0x100, {0x00})
end)

menu:addButton("Spawn Car", function()
    -- Get the car model from the user
    local carModel = input("Enter car model (0-100): ")
    -- Spawn the car
    writeBytes(0x004CARSPWN, {carModel})
end)

menu:addButton("Give Player Money", function()
    -- Get the player ID and money amount from the user
    local playerID = input("Enter player ID (0-7): ")
    local moneyAmount = input("Enter money amount: ")
    -- Write the money amount to the player's money address
    writeBytes(0x004MONEYADDR + playerID * 0x100, {moneyAmount})
end)

menu:addButton("Give Player Weapon", function()
    -- Get the player ID and weapon ID from the user
    local playerID = input("Enter player ID (0-7): ")
    local weaponID = input("Enter weapon ID (0-100): ")
    -- Write the weapon ID to the player's weapon address
    writeBytes(0x004WEAPONADDR + playerID * 0x100, {weaponID})
end)
